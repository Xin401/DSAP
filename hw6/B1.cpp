...
void Dictionary<TKey, TValue>::Delete(const TKey &key)
{
    ...
    {
        if (this->repr_[i].key == key)
        {
            this->repr_.erase(repr_.begin() + i);
        }
    }
}
...
std::vector<TKey> Dictionary<TKey, TValue>::Keys() const
{
    ...
    for (int i = 0; i < this->Size(); i++)
    {
        ans.push_back(this->repr_[i].key);
    }
    return ans;
}
...
std::vector<TValue> Dictionary<TKey, TValue>::Values() const
{
    ...
    for (int i = 0; i < this->Size(); i++)
    {
        ans.push_back(this->repr_[i].value);
    }
    return ans;
}
...
Dictionary<TKey, TValue> operator|(const Dictionary<TKey, TValue> &lhs, const Dictionary<TKey, TValue> &rhs)
{
    std::vector<KeyValuePair<TKey, TValue>> ans;
    Dictionary<TKey, TValue> rans;
    for (int i = 0; i < lhs.Size(); i++)
    {
        if (!rhs.Contains(lhs.KeyValuePairs()[i].key))
        {
            ans.push_back({lhs.KeyValuePairs()[i].key, lhs.KeyValuePairs()[i].value});
        }
    }
    for (int i = 0; i < rhs.Size(); i++)
    {
        ans.push_back({rhs.KeyValuePairs()[i].key, rhs.KeyValuePairs()[i].value});
    }
    for (int i = 0; i < ans.size(); i++)
    {
        rans[ans[i].key] = ans[i].value;
    }
    return rans;
}