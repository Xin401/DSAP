...
std::string Evaluate(std::string &cell, string table[1000][26], int selfl, int selfk, int l, int k, int mark[1000], int j);
...
int main()
{
    ...
    for (int l = 0; l < j; l++)
    {
        for (int k = 0; k <= mark[l]; k++)
        {
            if (table[l][k][0] != '#')
            {
                cout << Evaluate(table[l][k], table, -1, -1, l, k, mark, j);
                for (int pu = 0; pu < 1000; pu++)
                {
                    for (int ssy = 0; ssy < 26; ssy++)
                    {
                        fuck[pu][ssy] = 0;
                    }
                }
    ...
    if (cell[0] == '=')
    {
        if (cell.find_first_of("ABCDEFGHIJKLMNOPQRSTUVWXYZ") != string::npos)
        {
            ...
            if (fEng < op)
            {
                ...
                if (stoi(num1) - 1 == selfl && cell[fEng] - 65 == selfk)
                {
                    return "ERROR";
                }
                val1 = Evaluate(table[stoi(num1) - 1][cell[fEng] - 65], table, l, k, stoi(num1) - 1, cell[fEng] - 65, mark, j);
                if (val1.find("true") != string::npos || val1.find("false") != string::npos || val1.find("ERROR") != string::npos)
                {
                    return "ERROR";
                }
                ...
                if (cell.find_first_of("ABCDEFGHIJKLMNOPQRSTUVWXYZ", op + 1) != string ::npos)
                {
                    ...
                    if (stoi(num2) - 1 == selfl && cell[sEng] - 65 == selfk)
                    {
                        return "ERROR";
                    }
                    val2 = Evaluate(table[stoi(num2) - 1][cell[sEng] - 65], table, l, l, stoi(num2) - 1, cell[sEng] - 65, mark, j);
                ...
                return Evaluate(formula, table, -1, -1, l, k, mark, j);
            }
            else
            {
